...
impl SourceLocation {
    ...
    /// Check if this location's file matches a header filter string.
    ///
    /// Compares the lowercased full path with `ends_with`.
    #[must_use]
    pub fn matches_header(&self, filter: &str) -> bool {
...
/// Check if an entity's source file matches a header filter string.
///
/// Extracts the entity's file location and compares the lowercased full
/// path with `ends_with`. Used by all CLI filter structs as a pre-parse
/// filter (before constructing `SourceLocation`).
#[must_use]
pub fn entity_in_header(entity: &Entity, filter: &str) -> bool {
    let filter = filter.to_lowercase();
    entity
        .get_location()
