...
- **[`clang_ext`](./src/ext.rs)** — Extension traits over `clang-rs` types (`AnonymousType`, `DeclarationKind`, `UnderlyingType`, `HasChildrenType`). These are used throughout the crate to simplify working with the clang AST.
