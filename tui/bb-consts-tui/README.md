...
> TUI browser for **Windows SDK** / **PHNT** constants.

`bb-consts-tui` is a TUI version of the `bb-consts` CLI application crate, using its library code, and exposing the data that is gathered with it to a [`bb-tui`](../../crates/bb-tui/) data model.
...
Usage: bb-consts-tui.exe [OPTIONS]
...
Options:
  ...
  -n, --name <NAME>        Constant name pattern (supports * wildcard, Enum::Const syntax)
  -e, --enum <ENUM_NAME>   Enum name pattern (supports * wildcard)
...
**On the top** — you will see a search bar. Interact with it just as you would with `bb-consts`. It uses `*` as its match character, just like the aforementioned. You may scope your search to enums by containing `::` in your query string.

**On the left** — you will see a file tree, as a way to separate the enum and constant entities on the basis of their source location.

**On the right** — you will see the content area, displaying the enums and constants.
